...
import { X, ShieldCheck, Star, MessageCircle, Package, ArrowRight, Heart, AlertTriangle, Calendar, ShoppingBag, Info } from 'lucide-react';
...
export const TripDetailSheet: React.FC<TripDetailSheetProps> = ({ trip, onClose }) => {
  ...
  const { preferences, getPublicProfile } = useStore();
  ...
  const handleProfileClick = () => {
      HapticsService.impact('LIGHT');
      const profile = getPublicProfile(trip.traveler_uid);
  ...
  return (
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-t-[2.5rem] relative z-10 animate-slide-up shadow-2xl h-[90vh] flex flex-col overflow-hidden pointer-events-auto transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-32 no-scrollbar">
            ...
            {trip.latest_handoff_date && (
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl flex items-start gap-3 border border-blue-100 dark:border-blue-800 mb-6">
                    <Calendar className="w-5 h-5 text-moover-blue shrink-0 mt-0.5" />
                    <div>
                        <div className="text-xs font-bold text-moover-blue uppercase tracking-wider mb-0.5">Drop-off Deadline</div>
                        <div className="font-bold text-moover-dark dark:text-white">{trip.latest_handoff_date}</div>
                        <div className="text-xs text-gray-500 mt-1">Packages must be handed off by this date.</div>
                    </div>
                </div>
            )}

            {/* SHOPPER MODE STATUS (NEW SECTION) */}
            {trip.willing_to_buy ? (
                <div className="bg-green-50 dark:bg-green-900/10 p-4 rounded-xl flex items-start gap-3 border border-green-100 dark:border-green-800 mb-6">
                    <ShoppingBag className="w-5 h-5 text-green-600 shrink-0 mt-0.5" />
                    <div>
                        <div className="text-xs font-bold text-green-600 uppercase tracking-wider mb-0.5">Shopper Mode Available</div>
                        <div className="text-xs text-green-800 dark:text-green-200">
                            This traveler is willing to buy items for you. Reimbursement required.
                        </div>
                    </div>
                </div>
            ) : (
                <div className="bg-orange-50 dark:bg-orange-900/10 p-4 rounded-xl flex items-start gap-3 border border-orange-100 dark:border-orange-800 mb-6">
                    <Package className="w-5 h-5 text-orange-500 shrink-0 mt-0.5" />
                    <div>
                        <div className="text-xs font-bold text-orange-600 uppercase tracking-wider mb-0.5">Courier Only</div>
                        <div className="text-xs text-orange-800 dark:text-orange-200">
                            This traveler is <strong>not</strong> willing to buy items. You must ship the item to them or meet for handoff.
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-gray-50 dark:bg-zinc-800 p-5 rounded-2xl border border-gray-100 dark:border-zinc-700">
                    <div className="flex items-center gap-2 mb-2">
                        <Package className="w-5 h-5 text-moover-blue" />
                        <span className="text-xs font-bold text-gray-400 uppercase">{t('capacity')}</span>
                    </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 bg-white dark:bg-zinc-900 border-t border-gray-100 dark:border-zinc-800 z-30 pb-safe-bottom">
            <div className="flex gap-3">
                <button onClick={handleChatPress} className="flex-1 py-4 bg-gray-100 dark:bg-zinc-800 text-moover-dark dark:text-white font-bold rounded-2xl active:scale-95 transition-transform flex items-center justify-center gap-2">
                    <MessageCircle className="w-5 h-5" /> Chat
                </button>
                <button onClick={handleBookPress} className="flex-[2] py-4 bg-moover-dark dark:bg-white text-white dark:text-moover-dark font-bold rounded-2xl shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2">
                    Request to Carry <ArrowRight className="w-5 h-5" />
                </button>
            </div>
        </div>
      </div>
    ...
    {showOfferWizard && <OfferWizard trip={trip} onClose={() => setShowOfferWizard(false)} />}
    {showDelivery && <DeliveryCoordinationSheet tripId={trip.id} onClose={() => setShowDelivery(false)} />}
    {showChat && <ChatSheet tripId={trip.id} travelerName={trip.traveler_name} onClose={() => setShowChat(false)} />}
